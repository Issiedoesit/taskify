      ...
      >
        ...
        <div className="relative lg:static w-full h-full overflow-hidden lg:overflow-visible bg-white border-t-0.5 border-t-brandGray4x/20">
          <div
            className={`flex ${
              translate ? "h-full" : ""
            } lg:grid lg:grid-cols-6 h-full pb-20 w-full lg:w-full transition-transform duration-300 ease-in-out ${
              translate ? "-translate-x-hundredPercent lg:translate-x-0" : ""
            }`}
          >
            {/* message list */}
            <div className={`lg:col-span-2 w-full min-w-full px-4 border-r-0.5 border-r-brandGray4x/20 `}>
              <div className='py-8'>
                    <label htmlFor='searchMsgs' className='sticky top-0 left-0 z-20 bg-white border-0.5 border-brandGray4x/20 rounded-ten px-4 flex flex-row items-center gap-2'>
                        <input
                            type="search"
                            name="searchMsgs"
                            id="searchMsgs"
                  ...
                  <div className={`flex flex-col py-6 h-full overflow-y-auto`}>
                      {
                        messagesData?.map((message, idx) => {
                          return <div key={idx} onClick={()=>openMessage(message)} className={`flex flex-row gap-4 cursor-pointer w-full py-4 px-2 rounded-ten hover:bg-brandBlue1x/10`}>
                            <UserImg src={message.project_photo} width={"w-12 h-12"} minWidth={'min-w-12'} />

                            <div className="flex flex-col gap-1">
                              <p className={`text-sm font-avenirHeavy text-brandSec500`}>{message.name.charAt(0).toUpperCase() + message.name.slice(1)}</p>
                              <p className="text-brandGray4x font-avenirLight text-xs two-lined-text">{message?.project_message?.[0]?.message}</p>
                            </div>
                          </div>
                        })
                      }
            ...
            {/* message view */}
            <div className="lg:col-span-4 flex flex-col w-full min-w-full h-full relative pb-10">
                ...
                <>
                {
                  sortedMessages?.length == 0 ? (
                    ...
                    :
                    <div className="flex flex-col gap-4 py-4 px-4 h-fit w-full min-w-full">
