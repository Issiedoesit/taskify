...
const AuthSelect = ({display, colSpan, rowSpan, labelColor, handleBlur, fieldError, selectValue, labelFont, labelFontSize, handleChange, la, selectName, selectRequired, selectPlaceholder, selectId, paddingY, paddingX, children, fontSize}) => {
  return (
    <fieldset className={`gap-2.5 flex flex-col ${colSpan ? colSpan : 'col-span-1'}`}>
        <label htmlFor={selectId || 'country'} className={`${labelFontSize ? labelFontSize : 'text-sm'} ${labelFont ? labelFont : 'font-spaceGroteskMedium'} ${labelColor ? labelColor : 'text-brandGray14x'}`}>{la || "What's your fullname"}</label>
