...
const TaskCardWrap = ({ taskImg, name, description, creator, members, date, maxWidth }) => {
    return (
        <div className={`w-full rounded-ten shadow-md ${maxWidth ? maxWidth : "md:max-w-xs"}`}>
            ...
            <div className={`p-4 flex flex-col gap-2`}>
                <p className='text-xl font-avenirHeavy'>{name || "Short Task Name"}</p>
                <p className='text-sm two-lined-text text-brandGray1x'>{name || "2 line of description text in small font with text wrap if longer the containing element as shown here?"}</p>
            </div>
            <div className={`flex flex-col gap-1 p-4`}>
                <p className={`text-xs`}> Created by:</p>
                <div className={`flex flex-row gap-8 justify-between items-center`}>
                    <p className={`text-brandBlue1x font-avenirMedium`}>{creator?.name || "Jim Create"}</p>
                    {
