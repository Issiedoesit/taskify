...
const MessageDashTemplate = ({ messageView, children }) => {
  ...
  return (
    <div className={`flex w-full bg-brandDashGray1x min-h-full h-screen overflow-y-hidden`}>
      <Nav menuRef={menuRef} isOpen={isOpen} setIsOpen={setIsOpen} />

      {/* large screen */}
      <div className='flex flex-col z-20 w-full h-screen overflow-hidden bg-brandDashGray1x'>
        ...
        <div className={`${messageView ? "h-full overflow-y-hidden pb-20" : 'pb-48 lg:pb-24 pt-8 px-8 md:px-10 overflow-y-auto h-full min-h-full'}  bg-brandDashGray1x w-full flex flex-col max-w-full overflow-x-hidden`}>
        {children}
        </div>
      ...
      {/* mobile */}
      <div className='z-20 w-full hidden min-h-screen bg-brandDashGray1x'>
        <div className={`h-60px z-50 ${scrollUp && "sticky top-0 left-0 bg-brandDashGray1x shadow-md"}  xl:hidden border-b-0.5 border-b-brandGray2x`}>
          <div className='flex xl:hidden h-full items-center justify-between gap-10 px-8 md:px-10'>
            <NavLink to="/" className=''>
              <img src={Logo} alt='logo' className='w-7' />
...
export default MessageDashTemplate