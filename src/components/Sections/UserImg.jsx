...
const UserImg = ({src, width, alt, minWidth}) => {
  return (
    <>
        {
            src
            ?
            <img src={src} className={`skeleton--white rounded-full ${width ? width : "w-12"} ${minWidth ? minWidth : "w-12"} aspect-square object-cover`} alt={alt} />
            :
            <div className={`${width ? width : "w-12"} ${minWidth ? minWidth : "w-12"} rounded-full aspect-square flex items-center justify-center bg-brandBlue1x/50`}>
