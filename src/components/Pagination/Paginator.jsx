...
const Paginator = ({id, currentPage, totalPages, dottedArray, movePageBy, paginate, btnSize, arrowSize, arrowBtnSize, justifySection, fontSize}) => {
  // console.log(dottedArray)
  return (
    <div id={id} className={`flex flex-row flex-wrap gap-2 items-center ${justifySection ? justifySection : "justify-center lg:justify-end"}`}>
        <button title={`${currentPage == 1 ? 'You\'re on the First Page' : `Go To Page ${currentPage - 1} of ${totalPages}`}`} onClick={()=>currentPage == 1 ? null : movePageBy(-1)} className={`${currentPage == 1 ? 'bg-brandGray20x/20 cursor-not-allowed' : 'bg-white border-2 border-brandGray26x cursor-pointer'} ${arrowBtnSize || 'py-2.5 px-2 sm:py-4 sm:px-4 md:px-5'} rounded-six`} type='button'>
